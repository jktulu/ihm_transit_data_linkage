...
dates <- seq.Date(as.Date("2015-10-05"), as.Date("2015-10-11"), by = "day")
...
for (n in 1:length(dates)) {
  ...
  afcOneDay <- 
    dbGetQuery(con, 
               glue("select t1.record_id, t2.tt_operator, t1.isam_op_code,
                       ...
                       t1.etm_service_number, t1.transaction_datetime, 
                       t1.record_id) 
                       from afc t1 left join afc_avl_tt_link.
                       afc_avl_tt_link_{gsub('-', '_', date_p)}_ t2
                       using (record_id) where t1.transaction_datetime 
  ...
  afcOneweek <- 
    dbGetQuery(con, 
               ...
               extract('dow' from transaction_datetime) as dow, etm_service_number, 
               start_stage as start_stage_hex from afc where 
               transaction_datetime between '{
  ...
  for (k in 1:length(unique(afcOneDay$id))) {
    ...
    #                     TCVW stages rise outbound and decrease inbound
    ...
    results[[length(results) + 1]] <- afcOneDay_sealer %>% 
      ...
      select(id, record_id, start_stage, tt_operator, isam_op_code,
             etm_service_number, transaction_datetime, direction, case2)
      ...
      if (length(unique(afcOneDay_sealer_sub$start_stage)) == 1) {
        ...
        if (nrow(afcOneDay_sealer_sub[
          ...
          afcOneDay_sealer_sub[
            which(afcOneDay_sealer_sub$start_stage == 
                    afcOneDay_sealer_params2a$start_stage[1]),]$case <- dir2.p
        }
        ...
        if (nrow(afcOneDay_sealer_sub[
          ...
          afcOneDay_sealer_sub[
            which(afcOneDay_sealer_sub$start_stage == 
                    afcOneDay_sealer_paramsa$start_stage[1]),]$case <- dir1.p
        }
        
        
        results[[length(results) + 1]] <- 
          afcOneDay_sealer_sub %>% 
          select(id, record_id, tt_operator,
        ...
        for (i in 2:length(stages_collapse)) {
          if (stages_collapse[i] < stages_collapse[i - 1]) {
            result_collapse[i] <- dir1.p
          } else {
            result_collapse[i] = dir2.p
          }
        }
        ...
        if (stages_collapse[1] <= stages_collapse[2]) {
          result_collapse[1] <- dir2.p
        } else {
          result_collapse[1] <- dir1.p
        }
        ...
        if (length(afcOneDay_sealer_sub[
          ...
          afcOneDay_sealer_sub[
            which(afcOneDay_sealer_sub$start_stage == 
                    afcOneDay_sealer_paramsa$start_stage[1]),
            ]$result_collapse <- dir1.p
        }
        
        results[[length(results) + 1]] <- 
          afcOneDay_sealer_sub %>%
          select(id, record_id, start_stage, tt_operator,
                 isam_op_code,etm_service_number, transaction_datetime,
                 direction, case2 = result_collapse)
      }
    }
  }
  ...
  npos <- results.final %>% filter(!is.na(direction)) %>% nrow()
  ntrue <- results.final %>% filter(direction == case2) %>% nrow()
  
  print(glue("The match rate for this approach was {round(ntrue/npos*100,4)}%"))
}


# Examine the output to ensure that it meets expectations
results.final[which(results.final$isam_op_code == '????' & results.final$etm_service_number == '37'),] %>% View
results.final %>% head



# Diagnostics 1: Based on individual isam_op_code, etm_service_number sets.

t1 <- sqldf("select id, isam_op_code, etm_service_number, count(*) as count 
            from \"results.final\" where direction is not null 
            group by id, isam_op_code, etm_service_number",
            drv = "SQLite")

t2 <- sqldf("select id, isam_op_code, etm_service_number, count(*) as count 
            from \"results.final\" where direction=case2 
            group by id, isam_op_code, etm_service_number",
            drv = "SQLite")

t1 %>%
  left_join(t2, by = c("id", "isam_op_code", "etm_service_number")) %>% 
  mutate(prop = count.y/count.x) %>%
  filter(etm_service_number == '97') %>% 
  View("individual_97")

# Diagnostics 2: Based on etm_service_numbers. (useful for testing association
# between stage progression and direction.)

t1 <- sqldf("select isam_op_code, etm_service_number, count(*) as count
            from \"results.final\" where direction is not null 
            group by isam_op_code, etm_service_number",
            drv = "SQLite")

t2 <- sqldf("select isam_op_code, etm_service_number, count(*) as count 
            from \"results.final\" where direction=case2 
            group by isam_op_code, etm_service_number",
            drv = "SQLite")

t1 %>% 
  left_join(t2, by = c("isam_op_code", "etm_service_number")) %>%
  mutate(prop = count.y/count.x) %>% View('hjhj') %>% 
  filter(tt_operator == '????', etm_service_number == "2") %>%
  View
